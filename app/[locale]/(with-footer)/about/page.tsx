...
export async function generateMetadata({ params: { locale } }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      description: t('description'),
      url: canonicalUrl,
      siteName: 'Spongebob Tower Defense',
      type: 'article',
    },
    twitter: {
...
export default async function AboutPage({ params: { locale } }: Props) {
  ...
  return (
    <article className='max-w-4xl mx-auto px-4 py-16 prose prose-lg'>
      ...
      <section className='mt-12'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
          {Object.entries(t.raw('content.features')).map(([key, feature]: [string, any]) => (
            <div key={`feature-${key}`} className='bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md'>
      ...
      <section className='mt-12'>
        ...
        <div className='space-y-6'>
          {t.raw('content.howToPlay').map((step: any) => (
            <div key={`step-${step.title}`} className='bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md'>
      ...
      <section className='mt-12'>
        <h2 className='text-3xl font-bold mb-6'>{t('content.popularityTitle')}</h2>
        <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
          {t.raw('content.popularity').map((item: any) => (
            <div key={`popularity-${item.title}`} className='bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md'>
      ...
      <section className='mt-12'>
        <h2 className='text-3xl font-bold mb-4'>{t('content.codesTitle')}</h2>
        <p>{t('content.codes')}</p>
      </section>
      ...
      <section className='mt-12'>
        ...
        <div className='space-y-6'>
          {t.raw('content.tips').map((tip: any) => (
            <div key={`tip-${tip.title}`} className='bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md'>
